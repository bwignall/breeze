...
class QuasiMonteCarloTest extends AnyWordSpec {
  ...
  "quasiMonteCarloIntegrate" should {
    "approximate the mean of a normal distribution" in {
      val integrationResult = quasiMonteCarloIntegrate(identityFunc)(Gaussian(3, 5).toQuasi)(128 * 1024)
      math.abs(integrationResult - 3) should be < 0.001
    }
    "approximate the mean of a gamma distribution, alpha == 1" in {
      val integrationResult =
        quasiMonteCarloIntegrate(identityFunc)(RejectionSampledGammaQuasiRandomVariable(1, 4))(128 * 1024)
    ...
    "approximate the mean of a gamma distribution, alpha > 1" in {
      ...
      val beta = 5.0
      val integrationResult =
        quasiMonteCarloIntegrate(identityFunc)(RejectionSampledGammaQuasiRandomVariable(alpha, beta))(128 * 1024)
    ...
    "approximate the mean of a product distribution" in {
      ...
      val sigma = 5.0
      val integrationResult =
        quasiMonteCarloIntegrate(productFunc)(Gaussian(mu, sigma).toQuasi,
