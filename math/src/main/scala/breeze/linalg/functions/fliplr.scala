...
object fliplr extends UFunc {
  ...
  implicit def implDM[T]: Impl[DenseMatrix[T], DenseMatrix[T]] = (v: DenseMatrix[T]) => {
    if (!v.isTranspose) {
...
object flipud extends UFunc {
  ...
  implicit def implDV[T]: Impl[DenseVector[T], DenseVector[T]] = (v: DenseVector[T]) => {
...
object rot90 extends UFunc {
  ...
  implicit def impl1fromImpl2[T, R](implicit impl2: Impl2[T, Int, R]): Impl[T, R] = (v: T) => impl2(v, 1)
}
