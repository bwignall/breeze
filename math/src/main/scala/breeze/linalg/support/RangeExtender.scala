...
object RangeExtender {
  val All: Range.Exclusive = Range(0, -1, 1)
}
    ...
    if (re.isInclusive) {
      val (actualStart: Int, actualEnd: Int) =
        (
          ...
          else re.start, // actualStart will be given as argument to inclusive range "to"
          if (re.end < 0) totalLength + re.end
          else re.end // actualEnd will be given as argument to inclusive range "to"
        )
