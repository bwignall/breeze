...
object CompetitiveLinking extends BipartiteMatching {
  ...
  def extractMatching(matchingPotentials: Seq[Seq[Double]]): (Seq[Int], Double) = {
    ...
    val predsIt = for {
      (arr, i) <- matchingPotentials.iterator.zipWithIndex
      (w, j) <- arr.iterator.zipWithIndex
    } yield Prediction(i, j, w)
    ...
    var score = 0.0
    for {
      pred <- preds.iterator.takeWhile(_ => leftSet.size < n || rightSet.size < m)
    ...
    (ArraySeq.unsafeWrapArray(matching), score)
  }
