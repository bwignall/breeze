...
class CanTraverseValuesBenchmark extends BreezeBenchmark with BuildsRandomVectors {
  ...
  def timeMaxMatrixCols(reps: Int): Transpose[DenseVector[Double]] = runWith(reps, { randomMatrix(40, 1024) }) { arr =>
    max(arr(::, *))
  }

  def timeMaxMatrixRows(reps: Int): DenseVector[Double] = runWith(reps, { randomMatrix(40, 1024) }) { arr =>
    max(arr(*, ::))
  }

  def timeMinMatrixCols(reps: Int): Transpose[DenseVector[Double]] = runWith(reps, { randomMatrix(40, 1024) }) { arr =>
    min(arr(::, *))
  }

  def timeMinMatrixRows(reps: Int): DenseVector[Double] = runWith(reps, { randomMatrix(40, 1024) }) { arr =>
    max(arr(*, ::))
  }
